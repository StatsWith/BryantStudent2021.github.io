...
7. Find the average number of cases per da`y. Find the maximum cases a day.  
```{r}
...
```{r}
df=read.csv("C:\\Users\\student\\Documents\\data science\\Math 421\\Math 421\\train.csv")
head(df)
str(df)
names(df)
mean(df$Age)
...
```{r}
df2=ifelse(df$Fare<quantile(df$Fare)[4],yes = "Cheap",no = "Expensive")
df2=cbind(df2,df)
head(df2)
```
...
for (x in df2$Name){
  if (is.na(x = x)){
    ...
    y=as.character(strsplit(x,split = ",")[[1]][1])
  }
  Last=append(Last,y)
}
df2$Last=Last
